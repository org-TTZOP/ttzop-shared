...
const UI_T = {
  ...
  en: {
    book_title: "Book {name}", book_date: "Date", book_time: "Time", book_loading: "Looking for free time…", book_none: "No free time on this day", book_confirm: "Book {d} at {t}?", book_done: "Done! You are booked for {d} at {t}. Details are in your account.", book_taken: "That time was just taken. Please pick another.", book_login: "Sign in to your account to book", book_err: "Something went wrong. Please try later", book_seats: "Seats:", book_full: "No seats left — pick another time", grp_upto: "Group up to {n}", pe_title: "Edit photo", pe_crop_free: "No crop", pe_bright: "Brightness", pe_sat: "Saturation", pe_rotate: "Rotate", pe_flip: "Mirror", pe_contrast: "Contrast", pe_warm: "Warmth", pe_sharp: "Sharpness", pe_blur: "Blur", pe_vig: "Vignette", pe_reset: "Reset", pe_skip: "No edits", pe_skip_all: "No edits for all", pe_done: "Done", pe_cancel: "Cancel", trpack_sub_label: "Your site's subdomain (who receives the translations)", form_err_trpack: "Enter the site subdomain for the translation pack", dep_next: "Next departure", dep_left: "seats left", tk_title: "Ticket", tk_valid: "Valid", tk_cancelled: "Cancelled", tk_past: "Past", tk_notfound: "Ticket not found", tk_seats: "Seats", route_teaser: "Route ({n}) · ~{t}", route_plan: "Route plan", route_stop: "Stop", route_transfer: "🚶 Transfer ~{t}", route_gmaps: "Open the route in Google Maps", unit_h: "h", unit_min: "min", show_more:'Show more', show_less:'Hide', view_toggle:'Cards / List',
...
// нармалізатар вокладак (ААП-аўдыт 2026-07-18: было 3 копіі): радок → {u}; {u,f,c} — як ёсць
const _covsNorm = list => (Array.isArray(list) ? list : []).filter(Boolean).map(c => typeof c === 'string' ? { u: c } : c);
...
// 📅 БЛІЖЭЙШЫ ВЫЕЗД на картках групавых паслуг: адзін публічны запыт next_departures на ўсе .dep-line
// старонкі → «Бліжэйшы выезд: сб 19 ліп, 10:00 · вольных месцаў: 5»; клік = мадалка броні на ТОЙ дзень.
// Няма выездаў (ці паслуга не фіксаваная) — радок проста не паказваецца.
let _depReqSeq = 0;
async function _depBadgesInit() {
  const els = Array.from(document.querySelectorAll('.dep-line[data-dep]'));
  if (!els.length) return;
  const ids = [...new Set(els.map(e => e.dataset.dep))];
  const seq = ++_depReqSeq; // перарэндэр перагнаў адказ → не пісаць у мёртвыя вузлы
  let deps = {};
  try {
    const r = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'next_departures', repo: SITE_REPO, ids }) });
    deps = (await r.json())?.deps || {};
  } catch {}
  if (seq !== _depReqSeq) return;
  const ui = getUI();
  els.forEach(el => {
    const d = deps[el.dataset.dep]; if (!d || !el.isConnected) return;
    let dt = d.date;
    try { dt = new Date(d.date + 'T00:00:00').toLocaleDateString(currentLang === 'by' ? 'be' : currentLang, { weekday: 'short', day: 'numeric', month: 'short' }); } catch {}
    el.textContent = `📅 ${ui.dep_next || ''}: ${dt}, ${d.time} · ${ui.dep_left || ''}: ${d.left}`;
    el.style.display = '';
    el.onclick = ev => { ev.stopPropagation(); bookItem(el.dataset.dep, el.dataset.nm || '', d.date); };
  });
}
...
function _cardSlidersInit() {
  document.querySelectorAll('.card-slider:not([data-init])').forEach(el => {
    ...
    covs = _covsNorm(covs); // сумяшчальнасць: радкі і аб'екты {u,f,c}
    ...
    .forEach(n => {
      ...
      if (n.type === 'form') {
        ...
        items.push({ id: n.id, icon: f.icon || '', title: f.name || n.name || '', text: f.description || '',
          price: f.price != null ? String(f.price) : '', currency: f.currency || '', itemType: +f.trPack > 0 ? 'trpack' : (f.type || 'general'), // 🌐 пазіцыя-пакет перакладаў → чэкаўт спытае паддамен-мэту
          ...
          ...(f.body ? { body: f.body } : {}), // 📝 WYSIWYG-цела → чытач пазіцыі (клік па картцы)
...
const SITE_VIEWS = {
  ...
  cards: inst => {
    ...
    const cardHtml = (it, _i) => {
      ...
      // 📅 бліжэйшы выезд з рэшткамі (запаўняецца пасля рэндэру адным запытам next_departures — _depBadgesInit)
      ...
      return _cardHtml({ cls: 'service-card', icon: _sv(it.icon) || '🔧', covers: it.photos, title: name, text: _sv(it.text), badge, footer: priceHtml + btn, // 📸 фота → вокладка/слайдэр (icon = фолбэк)
        onClick: (hasReader && !_dEdit) ? `openItemReader('${_dsEsc(rk)}')` : '', // 📖 клік = чытач (у edit-рэжыме — не, каб не замінаць праўцы)
...
// 🌐 пакет AI-перакладаў у кошыку → чэкаўт спытае паддамен сайта-мэты (крэдыты налічацца пры paid)
function cartHasTrPack() {
  return cart.some(i => i.type === 'trpack');
}
    ...
    modal.innerHTML = `
      <div style="background:#181c27;border:1px solid #2a2f45;border-radius:16px;max-width:480px;width:100%;overflow:hidden;position:relative">
        ...
        <div style="padding:24px">
          ...
          ${cartHasTrPack() ? `
          <div style="margin-bottom:16px"><!-- 🌐 пакет перакладаў: якому САЙТУ налічыць (voркер пры paid праверыць існаванне) -->
            <label style="display:block;font-size:0.8rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px">${getUI().trpack_sub_label}</label>
            <div style="display:flex;align-items:stretch">
              <input type="text" id="order-trpack-input" placeholder="yourname"
                style="flex:1;padding:10px 14px;background:#1e2335;border:1.5px solid #2a2f45;border-right:none;border-radius:8px 0 0 8px;color:#e8eaf0;font-family:'Manrope',sans-serif;font-size:0.95rem" />
              <span style="padding:10px 14px;background:#252b40;border:1.5px solid #2a2f45;border-radius:0 8px 8px 0;color:#6b7280;font-size:0.9rem;white-space:nowrap;display:flex;align-items:center">.ttzop.com</span>
            </div>
          </div>` : ''}
...
let trPackSub = ''; // 🌐 мэта пакета AI-перакладаў (existing-паддамен; НЕ desiredSubdomain — каб не будзіць site-creation/intake)
...
async function sendOrderCode() {
  ...
  if (cartHasTrPack()) { // 🌐 пакет: паддамен ІСНУЮЧАГА сайта-мэты (асобная зменная — каб не будзіць site-creation/intake галіны)
    const ts = document.getElementById('order-trpack-input')?.value.trim().toLowerCase().replace(/[^a-z0-9-]/g, '');
    if (!ts) { errEl.textContent = ui.form_err_trpack; return; }
    trPackSub = ts;
  }
...
// ── 🎫 БІЛЕТ (QR): ?ticket={id}[&p={participantId}] → дакумент-білет праз reader.js (style:'paper').
// QR кадуе гэты ж URL — сканаванне адкрывае старонку з ЖЫВЫМ статусам ад воркера (ticket_check;
// id — capability). Вердыкт: зялёны «Сапраўдны» / чырвоны «Ануляваны» / шэры «Мінулы».
...
async function _ticketPageOpen() {
  const q = new URLSearchParams(location.search);
  const id = q.get('ticket'); if (!id) return false;
  const pid = q.get('p') || '';
  const ui = getUI();
  let d = null;
  try {
    const r = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'ticket_check', repo: SITE_REPO, id, ...(pid ? { p: pid } : {}) }) });
    d = await r.json();
  } catch {}
  try { await _qrLibLoad(); } catch {}
  const url = location.origin + location.pathname + '?ticket=' + encodeURIComponent(id) + (pid ? '&p=' + encodeURIComponent(pid) : '');
  let qrSvg = '';
  try { if (window.qrcode) { const qr = window.qrcode(0, 'M'); qr.addData(url); qr.make(); qrSvg = qr.createSvgTag({ cellSize: 4, margin: 0, scalable: true }); } } catch {}
  const okT = !!(d && d.ok);
  const dead = !okT || ['cancelled', 'no_show'].includes(d.status);
  const past = okT && !dead && (d.status === 'completed' || Date.parse(d.date + 'T' + (d.time || '00:00')) < Date.now());
  const verdict = !okT ? { txt: ui.tk_notfound, c: '#ef4444' } : dead ? { txt: ui.tk_cancelled, c: '#ef4444' } : past ? { txt: ui.tk_past, c: '#9ca3af' } : { txt: ui.tk_valid, c: '#16a34a' };
  let dateTxt = okT ? d.date : '';
  try { if (okT && d.date) dateTxt = new Date(d.date + 'T00:00:00').toLocaleDateString(currentLang === 'by' ? 'be' : currentLang, { day: 'numeric', month: 'long', year: 'numeric', weekday: 'long' }); } catch {}
  const row = (k, v) => v ? `<div style="display:flex;gap:12px;margin-top:8px;font-size:1rem"><span style="opacity:0.55;min-width:96px">${_dsEsc(k)}</span><b>${_dsEsc(String(v))}</b></div>` : '';
  const seatsTxt = okT ? (d.seats ? String(d.seats) : (d.groupMax ? `${d.taken || 0} / ${d.groupMax}` : '')) : '';
  const body = `
    <div style="text-align:center;margin:4px 0 20px"><span style="display:inline-block;padding:9px 26px;border-radius:999px;background:${verdict.c};color:#fff;font-weight:700;font-size:1.1rem">${_dsEsc(verdict.txt || '')}</span></div>
    ${qrSvg ? `<div style="width:230px;margin:0 auto 22px;background:#fff;padding:14px;border:1px solid #e5e7eb;border-radius:14px">${qrSvg}</div>` : ''}
    ${okT ? row('', d.name) + row(ui.book_date || '', dateTxt) + row(ui.book_time || '', d.time) + row(ui.tk_seats || '', seatsTxt) : ''}
    <div style="margin-top:18px;font-size:0.8rem;opacity:0.5;word-break:break-all">${_dsEsc(id)}</div>`;
  replaceWithReaderDoc({
    title: (okT && d.serviceName) || ui.tk_title || 'Ticket', meta: ui.tk_title || '',
    bodyHtml: body, lang: currentLang, dir: document.documentElement.dir || 'ltr', style: 'paper',
    share: { mode: 'url', value: url, title: ui.tk_title || '' },
    labels: { share: ui.reader_share, pdfprint: ui.reader_pdfprint, close: ui.reader_close, copied: ui.reader_copied },
    fontControls: false, siteName: _sv(siteData?.company?.name) || document.title || 'TTZOP',
    closeHref: location.pathname });
  return true;
}

// ── 🗺 ЧЫТАЧ МАРШРУТУ ЭКСКУРСІІ (той жа reader.js, стыль site — «новы дакумент = новы білдэр cfg»):
// рэестр route-пазіцый (як _sitePostReg) + вертыкальны таймлайн кропак з адноснымі часамі + мапа ў мадалцы
const _siteRouteReg = {};
function _routeDur(route) { return (route || []).reduce((a, p) => a + (+p.stay || 0) + (+p.move || 0), 0); }
function _fmtDurSite(min) { // хвіліны → «2 гадз 35 хв» на мове наведвальніка
  const ui = getUI(); min = Math.max(0, Math.round(+min || 0));
  const h = Math.floor(min / 60), m = min % 60;
  return (h ? h + ' ' + (ui.unit_h || 'h') : '') + (h && m ? ' ' : '') + ((m || !h) ? m + ' ' + (ui.unit_min || 'min') : '');
}
function _routeGmapsUrl(route) { const pts = (route || []).filter(p => p.lat && p.lng).map(p => p.lat + ',' + p.lng); return pts.length > 1 ? 'https://www.google.com/maps/dir/' + pts.join('/') : ''; }
function _routeBodyHtml(it) { // цела ЧЫТАЧА ПАЗІЦЫІ: WYSIWYG-апісанне + фота-сетка + (умоўна) маршрут-таймлайн з мапай + кнопка дзеяння
  const ui = getUI(), route = it.route || [], acc0 = _themeColor('--color-accent', '#f97316');
  // 📝 поўнае апісанне (санітызаваны/адмінскі HTML — рэндэрым як ёсць, як цела паста)
  const richBody = _sv(it.body) ? `<div style="line-height:1.65;margin:0 0 20px">${_sv(it.body)}</div>` : '';
  ...
  const rows = route.map((p, i) => {
    ...
    const stay = +p.stay || 0, move = +p.move || 0;
    acc += stay + move;
...
function _routeReaderCfg(it) {
  const bg = _themeColor('--card-bg', '#fff'), acc = _themeColor('--color-accent', '#f97316');
  const fg = readerReadableOn(bg); // тэкст з ЯРКАСЦІ фону (як _postReaderCfg)
  const ui = getUI();
  const meta = (it.route || []).length
    ? (ui.route_plan || '') + ' · ' + (ui.route_teaser || '').replace('{n}', (it.route || []).length).replace('{t}', _fmtDurSite(_routeDur(it.route)))
    : [_sv(it.price) && _sv(it.priceMode) !== 'quote' ? `${_sv(it.price)} ${_sv(it.currency)}` : ''].filter(Boolean).join(''); // без маршруту — цана як meta (пуста = нічога)
  return { title: _sv(it.title), meta, bodyHtml: _routeBodyHtml(it),
    lang: currentLang, dir: document.documentElement.dir || 'ltr', style: 'site',
    colors: { bg, fg, accent: acc },
    labels: { share: ui.reader_share, pdfprint: ui.reader_pdfprint, close: ui.reader_close, copied: ui.reader_copied, window: ui.read_in_tab },
    fontControls: true, siteName: _sv(siteData?.company?.name) || document.title || 'TTZOP' };
}
function openRouteReader(id) {
  const it = _siteRouteReg[id]; if (!it) return;
  openReaderModal(_routeReaderCfg(it));
  setTimeout(() => _routeReaderMap(it), 80); // мапа — толькі ў мадалцы (акно чытача без Leaflet → там спасылка Google Maps)
}
function openItemReader(id) { openRouteReader(id); } // 📖 адзіны ЧЫТАЧ ПАЗІЦЫІ (WYSIWYG-цела + фота + маршрут) — імя па функцыі; route-цізер і клік карткі вядуць сюды
// нумараваны кругляш-маркер маршруту (у mapBoot акна — СВАЯ копія знарок: ізаляваны дакумент без нашых функцый)
const _rtNumIcon = (acc, n) => L.divIcon({ className: '', html: `<div style="width:26px;height:26px;border-radius:50%;background:${acc};color:#fff;font:700 13px/26px system-ui,sans-serif;text-align:center;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.4)">${n}</div>`, iconSize: [26, 26], iconAnchor: [13, 13] });
function _routeReaderMap(it) {
  ...
  pts.forEach((p, i) => L.marker([+p.lat, +p.lng], { icon: _rtNumIcon(acc, i + 1) }).addTo(map).bindPopup(_dsEsc(_sv(p.name) || '')));
...
function _dItemEdit(secId, key, idx, type) {
  ...
  m.innerHTML = `<div class="ds-fmbox"><div class="ds-fmbody">${rows}</div><div class="ds-fmfoot"><button class="ed-cancel" onclick="_dFModalClose()">${_svcEsc(getUI().ed_cancel || 'Адмяніць')}</button><button class="ed-save" onclick="_dFModalSave('${_dsEsc(secId)}','${_dsEsc(key)}',${idx},'${_dsEsc(type)}')">${_svcEsc(getUI().ed_save || 'Захаваць')}</button></div></div>`;
  document.body.appendChild(m);
...
// 📎 фота ў Каталог з Чарнавіка: выбар файлаў (multiple) → draft_src op=file (R2; пазіцыя → аўта-Папка «Фота»)
function _dSrcFilePick(parentId) {
  _dMenuClose();
  let inp = document.getElementById('dsrc-file-input');
  if (!inp) { inp = document.createElement('input'); inp.type = 'file'; inp.accept = 'image/*'; inp.multiple = true; inp.id = 'dsrc-file-input'; inp.style.display = 'none'; document.body.appendChild(inp); }
  inp.onchange = async () => {
    const files = Array.from(inp.files || []); inp.value = '';
    // ✂️ фота-рэдактар перад загрузкай (той жа агульны photoedit.js, што ў панэлі; подпісы — мова гледача)
    const u = getUI();
    const peL = { title: u.pe_title, crop_free: u.pe_crop_free, bright: u.pe_bright, sat: u.pe_sat, rotate: u.pe_rotate, flip: u.pe_flip, contrast: u.pe_contrast, warm: u.pe_warm, sharp: u.pe_sharp, blur: u.pe_blur, vig: u.pe_vig, reset: u.pe_reset, skip: u.pe_skip, skip_all: u.pe_skip_all, done: u.pe_done, cancel: u.pe_cancel };
    let skipAll = false;
    for (let f of files) {
      if (!skipAll && typeof openPhotoEditor === 'function') {
        const ed = await openPhotoEditor(f, { labels: peL, applyAllOption: files.length > 1 });
        if (ed === null) continue;
        if (ed.skipAll) skipAll = true;
        f = ed.file;
      }
      if (f.size > 4 * 1024 * 1024) continue; // ~4МБ ліміт воркера
      const data = await new Promise((res, rej) => { const r = new FileReader(); r.onload = e => res(e.target.result); r.onerror = rej; r.readAsDataURL(f); });
      const r = await _draftPost({ action: 'draft_src', op: 'file', repo: SITE_REPO, parentId, contentType: f.type || 'image/jpeg', data });
      if (!r?.ok) break; // памылка (MIME/ліміт) — не малацім далей
    }
    await _svcFetchTree(); _dReload(); // слайдэр карткі абновіцца з новымі фота
  };
  inp.value = ''; inp.click();
}
// 📝 WYSIWYG-цела пазіцыі з Чарнавіка: адзін схаваны носьбіт з data-edm="src:{id}::body" → існуючая мадалка Quill
function _dSrcBodyEdit(id) {
  _dMenuClose();
  const n = _dSrcNode(id); if (!n) return;
  let tmp = document.getElementById('dsrc-body-tmp');
  if (!tmp) { tmp = document.createElement('div'); tmp.id = 'dsrc-body-tmp'; tmp.style.display = 'none'; document.body.appendChild(tmp); }
  tmp.dataset.edm = 'src:' + id + '::body';
  tmp.innerHTML = _sv(n.fields?.body) || '';
  _edModalOpen(tmp);
}
...
function _dSrcEdit(id) { // ✎ мадалка палёў вузла Каталога — той жа ds-fmodal, што ў пазіцый
  ...
  const m = document.createElement('div'); m.id = 'ds-fmodal'; m.className = 'ds-fmodal';
  m.innerHTML = `<div class="ds-fmbox"><div class="ds-fmbody">${rows}</div><div class="ds-fmfoot"><button class="ed-cancel" onclick="_dFModalClose()">${_svcEsc(getUI().ed_cancel || 'Адмяніць')}</button><button class="ed-save" onclick="_dSrcModalSave('${_dsEsc(id)}')">${_svcEsc(getUI().ed_save || 'Захаваць')}</button></div></div>`;
  document.body.appendChild(m);
  m.addEventListener('mousedown', e => { if (e.target === m) _dFModalClose(); });
}
