...
function _routeBodyHtml(it) { // цела ЧЫТАЧА ПАЗІЦЫІ: WYSIWYG-апісанне + фота-сетка + (умоўна) маршрут-таймлайн з мапай + кнопка дзеяння
  ...
  const phs = Array.isArray(it.photos) ? it.photos.map(c => typeof c === 'string' ? { u: c } : c) : [];
...
function _dSrcFilePick(parentId) {
  ...
  inp.onchange = async () => {
    ...
    const u = getUI();
    const peL = { title: u.pe_title, crop_free: u.pe_crop_free, bright: u.pe_bright, sat: u.pe_sat, rotate: u.pe_rotate, vig: u.pe_vig, reset: u.pe_reset, skip: u.pe_skip, skip_all: u.pe_skip_all, done: u.pe_done, cancel: u.pe_cancel };
    let skipAll = false;
    for (let f of files) {
