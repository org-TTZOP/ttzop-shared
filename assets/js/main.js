...
function renderDynamicSections(data) {
  ...
  const instHtml = (inst, d, idx, sibN) => {
    ...
    const ebar = _dSecBar(inst.id, idx > 0, idx < sibN - 1, inst.enabled !== false, _dsProp(inst, 'collapsed') !== 'yes'); // ▶/▼ ▲▼ ● ⋯ (толькі edit); fold = пачатковы open
  ...
  const folderHtml = (f, d, idx, sibN) => {
    ...
    const ebar = _dSecBar(f.id, idx > 0, idx < sibN - 1, f.enabled !== false, !f.collapsed); // ▶/▼ ▲▼ ● ⋯ раздзела-Папкі; fold = пачатковы open
...
function _dEditInit() {
  _dEditRender();
  // ▶/▼ у панэльцы вузла сінхронны з фолдам НЕЗАЛЕЖНА ад шляху (кнопка ці клік па загалоўку):
  // 'toggle' не бурбуліць — ловім capture-ам; першы .ds-eb-fold у абгортцы = кнопка ЎЛАСНАГА бара
  // (бар рэндэрыцца ПЕРАД details, дзеці — унутры details і таму пазней у DOM-парадку)
  document.addEventListener('toggle', ev => {
    const dt = ev.target;
    if (!_dEdit || !dt || dt.tagName !== 'DETAILS' || !dt.classList.contains('ds-fold')) return;
    const box = dt.closest('[id^="sec-"]'); const b = box && box.querySelector('.ds-eb-fold');
...
// `fold` — ААП-парытэт з левым дыскложурам панэлі: true/false = пачатковы open фолда (кнопка ПЕРШАЯ, як у радку
// вузла Панелі); undefined = фолда няма (ФайлБлок) — кнопкі няма, бо няма за што згортваць
function _dSecBar(id, canUp, canDown, active, fold) {
  if (!_dEdit) return '';
  ...
  const fbtn = fold === undefined ? '' : `<button class="ds-eb-btn ds-eb-fold" onclick="event.stopPropagation();_dFoldToggle('${_dsEsc(id)}')" title="${_svcEsc(_dL('Разгарнуць / Згарнуць', 'Expand / Collapse'))}">${fold ? '▼' : '▶'}</button>`;
  ...
  return `<div class="ds-editbar" contenteditable="false">${fbtn}${dot}${mv(canUp, 'up', '▲')}${mv(canDown, 'down', '▼')}${info}<button class="ds-eb-btn ds-eb-menu" onclick="event.stopPropagation();_dSecMenu('${_dsEsc(id)}',this)" title="${_svcEsc(_dL('Меню', 'Menu'))}">⋯</button></div>`;
}
// перамкнуць фолд свайго вузла; стрэлку абнаўляе адзіны 'toggle'-слухач (агульны шлях з клікам па загалоўку)
function _dFoldToggle(id) {
  const box = document.getElementById('sec-' + id); const d = box && box.querySelector('details.ds-fold');
  if (d) d.open = !d.open;
}
