...
function _svcKindOf(f) { // той жа інфер, што панэльны _kindOf (легасі без поля kind)
  ...
  if (ff !== 'booking' && f.fulfil) return 'goods';
  if ((f._route || []).length) return 'tour';
    ...
    .forEach(n => {
      ...
      if (n.type === 'form') {
        ...
        items.push({ id: n.id, icon: f.icon || '', title: f.name || n.name || '', text: f.description || '',
          ...
          ...(_svcKindHas(f, 'group') && +f.groupMax > 0 ? { groupMax: +f.groupMax } : {}), // 👥 групавая пазнака (від мусіць мець такую здольнасць)
...
function _dEditRender() { // ніжняя панэль: падказка + ⓘ узроўню СТАРОНКІ (Сметніца верхніх секцый) + 🚀 (толькі пры dirty); параметры/парадак — у ⋯ самой секцыі
  ...
  w.innerHTML = `<span class="look-lbl" style="opacity:.8">🖊 ${_svcEsc(_dL('Секцыі: ● ▲▼ ⓘ ⋯ на старонцы', 'Sections: ● ▲▼ ⓘ ⋯ on page'))}</span><button class="ds-eb-btn ds-add-btn" onclick="_dAddMenu(null,this)" title="${_svcEsc(_dL('Дадаць секцыю/раздзел', 'Add section/folder'))}" style="margin-left:8px">➕ ${_svcEsc(_dL('Дадаць', 'Add'))}</button><button class="ds-eb-btn" onclick="_dSecInfo(null,this)" title="${_svcEsc(_dL('Старонка: інфа і Сметніца', 'Page: info & Trash'))}">ⓘ</button><button class="ds-eb-btn" onclick="_dTranslate()" title="${_svcEsc(_dL('AI-пераклад пустых моў старонкі', 'AI-translate empty languages'))}">🌐</button>${pubHtml}`;
}
// 🌐 AI-пераклад СТАРОНКІ з Чарнавіка: воркер draft_translate САМ збірае ml-палі чарнавіка і ўжывае
// пераклады (адзінае ядро _trCore з панэльным translate_fields — ліміты/крэдыты агульныя).
async function _dTranslate() {
  const ui = getUI();
  let info = null;
...
// 💬 просты toast аверлэя (генерычны — любое кароткае паведамленне Чарнавіка)
function _dToast(msg) {
  let el = document.getElementById('ds-toast');
  if (!el) { el = document.createElement('div'); el.id = 'ds-toast'; el.style.cssText = 'position:fixed;left:50%;bottom:64px;transform:translateX(-50%);z-index:100001;background:var(--surface,#181c27);color:var(--text,#e8eaf0);border:1px solid var(--accent,#f97316);border-radius:10px;padding:10px 18px;font-size:0.9rem;box-shadow:0 8px 24px rgba(0,0,0,0.4);max-width:80vw'; document.body.appendChild(el); }
...
async function _dSrcAdd(kind, parentId, itemKind) { // itemKind = від пазіцыі (ITEM_KINDS); fulfil выводзіць СЕРВЕР
  _dMenuClose();
  try {
    const r = await _draftPost({ action: 'draft_src', op: 'add', repo: SITE_REPO, kind, parentId: parentId || null, itemKind: itemKind || '' });
    const j = r && r.json ? await r.json().catch(() => null) : null;
    await _dReload();
    if (j && j.id) _dSrcEdit(j.id); // новы вузел безназоўны і НеАктыўны (🌑 канон) → адразу ✎ запоўніць
  } catch (e) {}
}
// віды пазіцыі ў Чарнавіку — люстэрка панэльнага ITEM_KINDS (id + іконка + подпіс мовы старонкі)
const _D_ITEM_KINDS = [
  ['goods',   '🛒', ['Тавар', 'Product']],
  ['service', '📅', ['Паслуга (запіс на час)', 'Service (booking)']],
  ['group',   '👥', ['Групавы занятак', 'Group session']],
  ['tour',    '🚌', ['Экскурсія / выезд', 'Tour / trip']],
  ['sub',     '🔁', ['Падпіска', 'Subscription']],
  ['inquiry', '💬', ['Запыт', 'Enquiry']],
];
function _dSrcTypePick(parentId, btn) { // «Што ствараем?» — від з каталога (сервер выведзе fulfil сам)
  const r = btn.getBoundingClientRect(); _dMenuClose();
  ...
  m.innerHTML = `<div class="ds-mi-sep">➕ ${_svcEsc(_dL('Што ствараем?', 'What to create?'))}</div>`
    + _D_ITEM_KINDS.map(([id, ico, lb]) => mi(ico, _dL(lb[0], lb[1]), `_dSrcAdd('item','${_dsEsc(parentId)}','${id}')`)).join('');
  document.body.appendChild(m);
  m.style.top = (r.bottom + 6 + scrollY) + 'px';
  m.style.left = Math.max(8, Math.min(r.right - m.offsetWidth + scrollX, scrollX + innerWidth - m.offsetWidth - 8)) + 'px';
  setTimeout(() => document.addEventListener('mousedown', _dMenuOutside, true), 0);
...
function _dSrcFields(n) {
  ...
  return [{ k: 'icon', label: F('icon') }, { k: 'name', label: F('name'), ml: 1 }, { k: 'description', label: F('descr'), ml: 1, area: 1 },
    ...
    // 🧾 ВІД пазіцыі замест голага «Спосабу атрымання»: від вядзе fulfil за сабой (сервер), і ён жа
    // вырашае, што пазіцыя ўмее (маршрут/групавасць) — кліент не збірае несумяшчальныя пары рукамі
    { k: 'kind', label: _dL('Від пазіцыі', 'Item kind'), opts: _D_ITEM_KINDS.map(([id, ico, lb]) => [id, ico + ' ' + _dL(lb[0], lb[1])]) },
...
function _dSrcEdit(id) { // ✎ мадалка палёў вузла Каталога — той жа ds-fmodal, што ў пазіцый
  ...
  const cur = k => k === 'name' && n.type === 'folder' ? n.name : (k === 'kind' ? _svcKindOf(f) : f[k]); // назва Папкі жыве на вузле; від — з інферам для легасі
