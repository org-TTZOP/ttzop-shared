...
const SITE_VIEWS = {
  ...
  cards: inst => {
    ...
    const cardHtml = (it, _i) => {
      ...
      if (Array.isArray(it.route) && it.route.length) { // 🗺 цізер маршруту: клік → чытач плана (reader.js)
        const rk = it.id || ('i' + _i); _siteRouteReg[rk] = it;
        btn += `<p style="margin:8px 0 0;font-size:0.8rem;opacity:0.85;cursor:pointer" onclick="event.stopPropagation();openRouteReader('${_dsEsc(rk)}')">🗺 <span style="text-decoration:underline dotted">${_dsEsc((ui.route_teaser || '').replace('{n}', it.route.length).replace('{t}', _fmtDurSite(_routeDur(it.route))))}</span></p>`;
      }
...
function _routeReaderCfg(it) {
  ...
  const ui = getUI();
  const meta = (ui.route_plan || '') + ' · ' + (ui.route_teaser || '').replace('{n}', (it.route || []).length).replace('{t}', _fmtDurSite(_routeDur(it.route)));
...
function _dSrcMenu(id, btn) { // тая ж панэльная сетка іконак: [Папка: +📂 +🗒] · ⧉ · ✎ · ✕(danger)
  ...
  const g1 = n.type === 'folder'
    ...
    : mi('+📎', _dL('Фота карткі (слайдэр)', 'Card photos (slider)'), `_dSrcFilePick('${_dsEsc(id)}')`);
  m.innerHTML = [
    g1,
