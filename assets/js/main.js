...
function initLookPreview(data) {
  // пер-сайтавы токен (settings.lookToken): панэлька толькі пры дакладным супадзенні — голы ?look=1 ігнаруецца
  const tok = new URLSearchParams(location.search).get('look');
  if (!tok || !data.lookToken || tok !== data.lookToken) return;
...
function _dSecMenu(id, btn) { // папавер: ● схаваць/паказаць · ✎ назва · 🎨 параметры (для гэтай секцыі)
  ...
  const noParams = sec && (sec.kind === 'folder' || sec.kind === 'file'); // раздзел/файл — без disp-мадэлі
  ...
  const params = noParams ? '' : _secCat().map(p => {
...
async function _dSecDup(id) { _dMenuClose(); const tok = new URLSearchParams(location.search).get('look'); try { await _draftPost({ action: 'draft_dup', repo: SITE_REPO, lookToken: tok, id }); await _dReload(); } catch (e) {} } // ⧉ клон вузла+паддрэва
...
function _dItemMenu(secId, key, idx, type, btn) { // ⋯ пазіцыі: ✎ Рэдагаваць · ⧉ Дубляваць · 🗑 Выдаліць (парытэт радка панэлі)
  if (document.getElementById('ds-menu')) { _dMenuClose(); return; }
  const mi = (icon, label, onclick) => `<button class="ds-mi" onclick="${onclick}">${icon} ${_svcEsc(label)}</button>`;
  const m = document.createElement('div'); m.id = 'ds-menu'; m.className = 'ds-menu';
  m.innerHTML = (_dItemFields(type).length ? mi('✎', _dL('Рэдагаваць', 'Edit'), `_dItemEdit('${_dsEsc(secId)}','${_dsEsc(key)}',${idx},'${_dsEsc(type)}')`) : '')
    + mi('⧉', _dL('Дубляваць', 'Duplicate'), `_dItemDup('${_dsEsc(secId)}','${_dsEsc(key)}',${idx})`)
    + `<button class="ds-mi ds-mi-del" onclick="_dItemDelete('${_dsEsc(secId)}','${_dsEsc(key)}',${idx})"><span class="ds-x">✕</span> ${_svcEsc(_dL('Выдаліць', 'Delete'))}</button>`; // канонны чырвоны ✕
  document.body.appendChild(m);
  const r = btn.getBoundingClientRect();
  m.style.top = (r.bottom + 6 + scrollY) + 'px';
  m.style.left = Math.max(8, Math.min(r.right - m.offsetWidth + scrollX, scrollX + innerWidth - m.offsetWidth - 8)) + 'px';
...
async function _dItemDup(secId, key, idx) { _dMenuClose(); const tok = new URLSearchParams(location.search).get('look'); try { await _draftPost({ action: 'draft_item', op: 'dup', repo: SITE_REPO, lookToken: tok, id: secId, key, idx }); await _dReload(); } catch (e) {} }
...
async function _dFModalSave(secId, key, idx, type) {
  ...
  _dFModalClose();
  const tok = new URLSearchParams(location.search).get('look');
  try { await _draftPost({ action: 'draft_item', op: 'set', repo: SITE_REPO, lookToken: tok, id: secId, key, idx, patch }); await _dReload(); } catch (e) {}
}
...
function _dAddMenu(parentId, btn) { // пікер «Дадаць» на ўзроўні старонкі (з ніжняй панэлі)
  if (document.getElementById('ds-menu')) { _dMenuClose(); return; }
  const m = document.createElement('div'); m.id = 'ds-menu'; m.className = 'ds-menu';
  m.innerHTML = `<div class="ds-mi-sep">➕ ${_svcEsc(_dL('Дадаць', 'Add'))}</div>${_dAddOptionsHtml(parentId)}`;
  document.body.appendChild(m);
  const r = btn.getBoundingClientRect();
  m.style.top = (r.bottom + 6 + scrollY) + 'px';
  m.style.left = Math.max(8, Math.min(r.right - m.offsetWidth + scrollX, scrollX + innerWidth - m.offsetWidth - 8)) + 'px';
...
async function _dAddNode(kind, secType, parentId) { // стварыць секцыю/раздзел у чарнавік
  _dMenuClose();
  const tok = new URLSearchParams(location.search).get('look');
  try { await _draftPost({ action: 'draft_add', repo: SITE_REPO, lookToken: tok, kind, secType, parentId: parentId || null }); await _dReload(); } catch (e) {}
}
async function _dItemAdd(secId, key, type) { // ➕ пустая пазіцыя → перарэндэр → адразу мадалка палёў новай
  _dMenuClose();
  const tok = new URLSearchParams(location.search).get('look');
  try {
    await _draftPost({ action: 'draft_item', op: 'add', repo: SITE_REPO, lookToken: tok, id: secId, key });
    await _dReload();
